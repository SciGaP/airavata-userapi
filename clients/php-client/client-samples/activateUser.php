...
try
{
    ...
    if($token !== null){
        ...
        if($client->isActiveUser("phpTestUser", $token)){
            print "New user is created as an active user\n";
            $client->deactivateUser("phpTestUser", $token);
            if(!$client->isActiveUser("phpTestUser", $token)){
                print "New user is deactivated successfully\n";
                $client->activateUser("phpTestUser", $token);
                if($client->isActiveUser("phpTestUser", $token)){
                    print "New user is re-activated successfully\n";
                    $client->removeUser("phpTestUser", $token);
                }else{
                    print "Error! New user was unable to re-activate\n";
                    $client->removeUser("phpTestUser", $token);
                    exit;
                }
            }else{
                print "Error! New user was unable to deactivate\n";
                $client->removeUser("phpTestUser", $token);
                exit;
            }
        }else{
            print "Error! New user was not created as an active user\n";
            $client->removeUser("phpTestUser", $token);
            exit;
        }
    }else{
